...
def Pose_Detected(cap, use_vedio, dir, count, text, accuracy):
    ...
    while True:
        ...
        if success:
            ...
            #angle1:手肘到肩膀到寬的角度
            angle1_1 = detector.findAngle(landmarks[14], landmarks[12],
                                            landmarks[24], img)
            angle1_2 = detector.findAngle(landmarks[13], landmarks[11],
                                            landmarks[23], img)
            
            #angle2:肩膀到髖到膝蓋的角度
            angle2_1 = detector.findAngle(landmarks[12], landmarks[24],
                                            landmarks[26], img)
            angle2_2 = detector.findAngle(landmarks[11], landmarks[23],
                                            landmarks[25], img)
            
            #angle3:髖到膝蓋到腳踝的角度
            angle3_1 = detector.findAngle(landmarks[24], landmarks[26],
                                            landmarks[28], img)
            angle3_2 = detector.findAngle(landmarks[23], landmarks[25],
                                            landmarks[27], img)
            
            #angle4:肩膀到手肘到手腕的角度
            angle4_1 = detector.findAngle(landmarks[12], landmarks[14],
                                            landmarks[16], img)
            angle4_2 = detector.findAngle(landmarks[11], landmarks[13],
                                            landmarks[15], img)
            '''
            #angle5:髖到髖到膝蓋的角度
            angle5_1 = detector.findAngle(landmarks[23], landmarks[24],
                                            landmarks[26], img)
            angle5_2 = detector.findAngle(landmarks[24], landmarks[23],
                                            landmarks[25], img)
            '''
